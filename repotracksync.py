...
def parseArgs():
    ...
    parser.add_option("-s", "--reposync", default=False, action="store_true",
                      help="Generate reposync.conf with includepkgs only")
...
# gpg/sha checksum them
    ...
    if opts.reposync:
        for pkg in download_list:
            if not repos.get(pkg.repo):
                repos[pkg.repo] = set([pkg.name])
            else:
                repos[pkg.repo] = repos[pkg.repo].union(set([pkg.name]))
        for repo, packages in repos.iteritems():
            repo_string = ''
            for line in StringIO.StringIO(repo.dump()):
